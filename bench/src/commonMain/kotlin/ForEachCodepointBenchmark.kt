package de.cketti.codepoints.benchmark

import de.cketti.codepoints.deluxe.codePointIterator
import de.cketti.codepoints.deluxe.codePointSequence
import de.cketti.codepoints.deluxe.forEachCodePoint
import de.cketti.codepoints.deluxe.forEachCodePointIndexed
...
class ForEachCodepointBenchmark {

    @Param("10", "400", "-1")
    var codepointsCount: Int = 0
    
    private lateinit var stringToIterate: String

    @Setup
    fun setup() {
        stringToIterate = if (codepointsCount < 0) {
            UNICODE_TEXT
        } else {
            val endIndex = findIndexForLastCodepoint()
            UNICODE_TEXT.substring(0, endIndex)
        }
    }

    private fun findIndexForLastCodepoint(): Int {
        var count = 0
        var endIndex: Int
        UNICODE_TEXT.forEachCodePointIndexed { index, codePoint ->
            count++
            if (count == codepointsCount) {
                endIndex = index + codePoint.charCount
                return endIndex
            }
        }
        return UNICODE_TEXT.length
    }

    @Benchmark
    fun forEachInline() : Long {
        var sum = 0L
        stringToIterate.forEachCodePoint {
            sum += it.value
        }
        return sum
    }
    ...
    @Benchmark
    fun forEachSequence() : Long {
        var sum = 0L
        stringToIterate.codePointSequence().forEach {
            sum += it.value
        }
        return sum
    }
    ...
    @Benchmark
    fun forEachIterator() : Long {
        var sum = 0L
        stringToIterate.codePointIterator().forEach {
            sum += it.value
        }
        return sum
    }
